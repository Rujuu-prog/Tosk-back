...
class AuthLoginInvalidEmailTest {
  @Autowired MockMvc mockMvc;
  @Autowired ObjectMapper om;

  @Test
  void loginWithUnknownEmailReturns400Validation() throws Exception {
    // ensure at least one user exists to avoid rate limiter noise
    SignupRequestDTO sr =
    ...
    LoginRequestDTO bad =
        LoginRequestDTO.builder().email("unknown@example.com").password("whatever").build();
    mockMvc
        .perform(
            post("/api/auth/login")
                .contentType(MediaType.APPLICATION_JSON)
